...
-- monthly_material_base = 1.00 es un placeholder (el dominio exige M > 0):
-- configurarlo con el valor real antes de costear productos.
...
2. **IDs autogenerados**: `GENERATED BY DEFAULT AS IDENTITY` (autoincremento con posibilidad de seeds explícitos). Excepciones: `units_of_measurement` (ids fijos 1-6 = orden del enum Java) y `cost_settings` (singleton id=1).

3. **Entidad débil (ingredients)**: no existe sin una Recipe. Borrado en cascada tanto en BD (`ON DELETE CASCADE`) como en JPA (`cascade = ALL, orphanRemoval = true`).
